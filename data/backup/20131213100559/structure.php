...
$db->exe("DROP TABLE IF EXISTS `".$db_prefix."a`");
$db->exe("CREATE TABLE `".$db_prefix."a` (
...
$db->exe("DROP TABLE IF EXISTS `".$db_prefix."a_data`");
$db->exe("CREATE TABLE `".$db_prefix."a_data` (
...
$db->exe("CREATE TABLE `".$db_prefix."config` (
  ...
  `type` enum('站点配置','高级设置','上传配置','会员设置','邮箱配置','安全设置','水印设置','内容相关') NOT NULL DEFAULT '站点配置' COMMENT '配置类型\n1 站点配置\n2 性能设置\n3 上传配置\n4 交互设置\n5 会员设置',
...
$db->exe("CREATE TABLE `".$db_prefix."field` (
  ...
  PRIMARY KEY (`fid`),
  KEY `mid` (`mid`)
) ENGINE=MyISAM AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='模型字段'");
...
$db->exe("CREATE TABLE `".$db_prefix."upload` (
  ...
  PRIMARY KEY (`id`),
  KEY `aid` (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=26 DEFAULT CHARSET=utf8 COMMENT='上传文件'");
