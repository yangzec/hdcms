...
$(function () {
    $("#mid").change(function () {
        ...
        for (var i in category) {
            ...
            if(mid>0){
                dis =category[i].cattype==1 ? "" : "disabled='disabled' class='disabled'";
            }

            html += "<option value='" + category[i].cid + "' " + dis + ">" + category[i].catname + "</option>";
        }
        $("#cid").html(html);
        //设置选项
        if (mid == 0) {
            $("tr:gt(2)").hide().find('input').attr("disabled", "disabled");
        } else {
            $("tr:gt(2)").show().find('input').removeAttr("disabled");
        }
    })
})
//更新
